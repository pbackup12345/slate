...
import { Path, Node as Node$1, Mark, Range, Element as Element$1, Value } from 'slate';
...
(function (Leaf) {
  ...
  Leaf.isLeaf = function (value) {
    return isPlainObject(value) && typeof value.text === 'string' && Mark.isMarkSet(value.marks) && Range.isRangeList(value.decorations) && Range.isRangeMap(value.annotations);
  };
  /**
...
const Children = (props) => {
    ...
    const children = [];
    const isLeafBlock = Element$1.isElement(node) && !editor.isInline(node) && editor.hasInlines(node);
                            ...
                            if (relatedTarget) {
                                ...
                                if (isDOMElement(relatedTarget)) {
                                    ...
                                    if (editor.hasDomNode(relatedTarget) &&
                                        (!Element$1.isElement(node) || !editor.isVoid(node))) {
                                        return;
                                    }
                                }
                            }
    ...
    function (_Editor) {
      ...
      createClass(ReactEditor, [{
        ...
        value: function findEventRange(event) {
          ...
          if (Element$1.isElement(node) && this.isVoid(node)) {
...
export { Editor as Editable, Editor, useEditor, useFocused, useReadOnly, useSelected, useSlate, withReact };
